...
{
    public sealed class GetOrganizedGameHistoryQueryHandlerTests
    {
        ...
        public async Task Handle_ShouldScopeToCurrentProfileAndForwardFiltersClockAndCancellation()
        {
            ...
            var expected = new PagedResult<OrganizedGameHistoryDto>([], 2, 10, 0);
            var parameters = new GameHistoryQueryParameters(profile.Id, now, 2, 10, GameHistoryPeriod.Past,
                GameStatus.Completed, from.ToUniversalTime(), to.ToUniversalTime(), courtId);
            _games.Setup(repository => repository.GetOrganizedHistoryAsync(parameters, token)).ReturnsAsync(expected);

            var result = await CreateHandler().Handle(new GetOrganizedGameHistoryQuery(2, 10, GameHistoryPeriod.Past,
                GameStatus.Completed, from, to, courtId), token);
        ...
        {
            Func<Task> action = () => CreateHandler().Handle(new GetOrganizedGameHistoryQuery(), CancellationToken.None);
        ...
        private GetOrganizedGameHistoryQueryHandler CreateHandler() => new(_games.Object, _profiles.Object, _currentUser.Object, _clock.Object);
    }
}
